...
{
   printf("\nspi-loader V1.0 copyright 2019 RHS Research LLC"
          ...
          "\n   -D: Dump flash contents to stdout"
...
int main(int argc, char *argv[])
{
   try
   {
      ...
      long int slave_id = 0;
      bool dump_flash = false;

      // Process command line args
      int option;
      while ((option = getopt(argc, argv, "a:b:l:d:r:f:m:vct:D")) != -1)
      {
         switch (option)
         {
         case 'a':
...
// << early exit
      ...
      if (verify || verifyonly)
      {
         ...
         for (size_t xx = 0; xx < real_num_to_verify; xx++)
         {
          ...
          {
            uint8_t this_byte = 0;
            printf("Flash contents read: Data from Flash vs. Data from Input File\n");
            for(size_t line = 0; line < real_num_to_verify; line += 16)
            {
                for(size_t byte = 0; byte < 16; byte++)
                {
                    if((line+byte) >= real_num_to_verify)
                        this_byte = 0;
                    else
                        this_byte = tmp[line+byte];
                    printf("%02X ", (unsigned int) this_byte);
                }
                printf("       0x%06X        ", (unsigned int) line);
                for(size_t byte = 0; byte < 16; byte++)
                {
                    if((line+byte) >= real_num_to_verify)
                        this_byte = 0;
                    else
                        this_byte = data_to_write[line+byte];
                    printf("%02X ", (unsigned int) this_byte);
                }
                printf("\n");
            }
            printf("End flash contents\n");
          }
