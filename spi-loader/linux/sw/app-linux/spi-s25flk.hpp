 ...
 */
   void Init(const XSpi_Config& cfg, int spiIndex)
   {
      ...
      // Set the library to use the specific slave
 ...
 */
   void EraseRange(uint32_t addr, size_t len)
   {
      ...
      while (erased_bytes < len)
      {
         ...
         erased_bytes += BLOCK_ERASE_BYTES;
 ...
 */
   void Read(uint32_t flash_addr, uint8_t* dst, const size_t len)
   {
      ...
      while (numread < len)
      {
         ...
         AddAddr(flash_addr);
         AddByte(0); // Dummy 8 clock cycles

         // Read up to one page
         const auto flash_page_bytes = FLASH_PAGE_BYTES + DUMMY_BYTES;  // Needed to compile C++11/C++14. Fixed in C++17. See https://stackoverflow.com/questions/8016780/undefined-reference-to-static-constexpr-char
         const size_t real_count = std::min(flash_page_bytes, len - numread + DUMMY_BYTES);
         const auto* rezbuf = Execute(real_count);

         memcpy(dst + numread, rezbuf + DUMMY_BYTES, real_count - DUMMY_BYTES);

         flash_addr += real_count - DUMMY_BYTES;
         numread += real_count - DUMMY_BYTES;
...
//--------------------------------------------------------------------------------
   ...
   {
      mWriteBuf[mCurrWriteBufInx++] = data;
   }
...
//--------------------------------------------------------------------------------
   ...
   uint8_t* Execute(size_t num2read, double timeout_s = FLASH_DEFAULT_CMD_TIMEOUT_S)
   {
      ...
      if (((mCurrWriteBufInx - 1) + num2read) >= TOTAL_BUFFER_SIZE + DUMMY_BYTES)
      {
...
//--------------------------------------------------------------------------------
   ...
   static constexpr size_t DUMMY_BYTES = 3;
   static constexpr size_t FLASH_MAX_CMD_BYTES = 5;
   static constexpr size_t BLOCK_ERASE_BYTES = 64 * 1024;

   // Commands
   static constexpr uint8_t CMD_BLOCK_ERASE	  = 0xD8;
   static constexpr uint8_t CMD_WRITE_ENABLE	  = 0x06;
   static constexpr uint8_t CMD_PAGEPROGRAM_WRITE = 0x02;
   static constexpr uint8_t CMD_RANDOM_READ       = 0x6B;  // Fast read quad- requires 8 dummy clocks after command

   static constexpr uint8_t CMD_STATUSREG_READ    = 0x05;  // status register 1
